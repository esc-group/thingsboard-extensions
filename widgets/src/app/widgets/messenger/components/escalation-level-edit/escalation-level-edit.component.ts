...
export class EscalationLevelEditComponent
  implements OnDestroy, OnInit, OnChanges
{
  ...
  public selectedLanguage: string =
    EscalationLevelEditComponent.DEFAULT_LANGUAGE;

  protected fcName = new FormControl<string>("", [
    Validators.required,
    Validators.minLength(1),
  ]);
  protected fcTimeout = new FormControl<number>(60, [
    Validators.required,
    Validators.min(10),
    Validators.max(3600),
  ]);
  ...
  protected fcDispatchTemplate = new FormControl<string>("", [
    Validators.required,
    Validators.minLength(1),
  ]);
  protected fcAcceptTemplate = new FormControl<string>("", [
    Validators.required,
    Validators.minLength(1),
  ]);
  protected fcCancelTemplate = new FormControl<string>("", [
    Validators.required,
    Validators.minLength(1),
  ]);
  ...
  protected formGroup: FormGroup = new FormGroup({
    ...
    [EscalationLevelEditComponent.FC_DISPATCH_TEMPLATE]:
      this.fcDispatchTemplate,
  ...
  @Output() status: Observable<FormControlStatus> =
  ...
  ngOnInit(): void {
    this.subscriptions.push(
      ...
      this.selectedLevel$
        .pipe(op.filter((level) => level !== null))
        .subscribe((level) => {
          ...
          if (this.fcTimeout.value != level.timeout)
            this.fcTimeout.setValue(level.timeout);
          ...
          if (
            this.fcDispatchTemplate.value !=
            level.dispatch[this.selectedLanguage]
          )
            this.fcDispatchTemplate.setValue(
              level.dispatch[this.selectedLanguage]
            );
          if (
            this.fcAcceptTemplate.value != level.accept[this.selectedLanguage]
          )
            this.fcAcceptTemplate.setValue(level.accept[this.selectedLanguage]);
          if (
            this.fcCancelTemplate.value != level.cancel[this.selectedLanguage]
          )
            this.fcCancelTemplate.setValue(level.cancel[this.selectedLanguage]);
        }),
      this.fcName.valueChanges
        .pipe(op.debounceTime(750))
        .subscribe(this.setLevelName),
      this.fcTimeout.valueChanges
        .pipe(op.debounceTime(750))
        .subscribe(this.setLevelTimeout),
      this.fcAcceptedTimeout.valueChanges
        .pipe(op.debounceTime(750))
        .subscribe(this.setLevelAcceptedTimeout),
      this.fcNext.valueChanges
        .pipe(op.debounceTime(750))
        .subscribe(this.setLevelNext),
      this.fcDispatchTemplate.valueChanges
        .pipe(op.debounceTime(750))
        .subscribe(this.setDispatchTemplate),
      this.fcAcceptTemplate.valueChanges
        .pipe(op.debounceTime(750))
        .subscribe(this.setAcceptTemplate),
      this.fcCancelTemplate.valueChanges
        .pipe(op.debounceTime(750))
        .subscribe(this.setCancelTemplate)
    );
  }
  ...
  deviceSelectionChange = (change: MatSelectionListChange): void => {
    change.options.forEach((option) => {
      const action = option.selected
        ? EscalationActions.addDeviceToLevel
        : EscalationActions.removeDeviceFromLevel;
  ...
  groupSelectionChange = (change: MatSelectionListChange): void => {
    change.options.forEach((option) => {
      const action = option.selected
        ? EscalationActions.addGroupToLevel
        : EscalationActions.removeGroupFromLevel;
  ...
  alarmSelectionChange = (change: MatSelectionListChange): void => {
    change.options.forEach((option) => {
      const action = option.selected
        ? EscalationActions.addAlarmToLevel
        : EscalationActions.removeAlarmFromLevel;
  ...
  setLevelAcceptedTimeout = (acceptedTimeout: number): void => {
    if (this.fcAcceptedTimeout.valid) {
      ...
      this.store.dispatch(
        EscalationActions.patchLevel({ patch: { acceptedTimeout } })
      );
    }
  };
